...
import (
	...
	"github.com/BrianBFarias/homebrew-spark/internal/git"
...
Examples:
  spark sync                # pull all repos
  spark sync BusinessAPI    # pull specific repo`,
...
func syncRepo(wsPath string, ws *workspace.Workspace, name string) error {
	...
	if !ok {
		return fmt.Errorf("repo '%s' not found in workspace — run 'spark list' to see registered repos", name)
	}

	repoDir := filepath.Join(wsPath, repo.Path)
	if _, err := os.Stat(repoDir); os.IsNotExist(err) {
		return fmt.Errorf("repo directory %s does not exist — run 'spark use' to clone it", repoDir)
	}
...
func syncAll(wsPath string, ws *workspace.Workspace) error {
	if len(ws.Repos) == 0 {
		fmt.Println("No repos in workspace — run 'spark use <org/repo>' to add one")
		return nil
	}
